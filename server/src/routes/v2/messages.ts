  ...
  try {
    ...
    for (const msg of messages) {
      ...
      if (!conversationMap.has(otherId)) {
        conversationMap.set(otherId, { user: otherUser, lastMessage: msg, unread: (!msg.isRead && msg.receiverId === req.user!.userId) ? 1 : 0 });
      } else {
        const conv = conversationMap.get(otherId)!;
        if (!msg.isRead && msg.receiverId === req.user!.userId) conv.unread++;
      }
    }
...
router.get('/unread/count', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({ count });
  } catch { res.status(500).json({ error: 'Failed to fetch unread count' }); }
});
...
router.post('/', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const message = await prisma.message.create({
      data: { content: data.content, senderId: req.user!.userId, receiverId, jobId: data.jobId },
      include: {
